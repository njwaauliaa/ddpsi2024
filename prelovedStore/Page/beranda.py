...
if "logged_in" not in st.session_state or not st.session_state.logged_in:
    ...
    # Jika pengguna berada di halaman beranda
       ...
       # CSS untuk styling warna
        ...
        st.sidebar.image('./Page/prelovedstore.jpg', caption="Promo Terbaik untuk Anda!", use_column_width=True)
        st.sidebar.markdown("---")
        ...
        st.image("./Page/prelovedstore.jpg", caption="Promo Terbaik untuk Anda!", use_column_width=True)
...
# Fungsi untuk menambah item ke keranjang
        ...
        for i, product in enumerate(products["produk rekomendasi"]):
            with cols[i % 5]:  # Menampilkan setiap produk di kolom masing-masing
                st.image(product["gambar"], use_column_width=True)
